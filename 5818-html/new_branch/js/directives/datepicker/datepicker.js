...
angular.module('admin').directive('datepicker', [
  '$rootScope',
  '$state',
  '$locale',
  '$location',
  ...
  function ($rootScope, $state, $locale, location, $filter, $modal, dateUtils, lessonService, bookService, unitService, courseService, coursewareService) {
    ...
    return {
      ...
      template: '<div class="pickadate-top"></div>' + '<div class="pickadate-header">' + '<div class="special"><button ng-click="prevMonth()" class="btn btn-primary">\u4e0a\u4e00\u4e2a\u6708</button></div>' + '<div class="special"><h3 class="pickadate-centered-heading">' + '{{yearMonth}}' + '</h3></div>' + '<div class="special"><button ng-click="nextMonth()" class="btn btn-primary">\u4e0b\u4e00\u4e2a\u6708</button></div>' + '</div>' + '<div class="pickadate-weeks">' + '<div class="pickadate-week" ng-repeat="week in dayName">{{week}}</div>' + '</div>' + '<div class="pickadate-body">' + '<div class="pickadate-main">' + '<ul class="pickadate-cell">' + '<li bindonce ng-repeat="d in dates" ng-click="showModal(d)">' + '<div class="{{d.className}}" >' + '<b>{{d.dateObj | date:"d"}}</b>' + '<span class="ellipsis margin-bottom-5">{{d.data.uname}}</span>' + '<p class="ellipsis">{{d.data.wname}}</p>' + '</div></li>' + '</ul>' + '</div>' + '</div>',
      link: function (scope, element, attrs, ngModel) {
        var weekStartsOn = scope.weekStartsOn || 0, noExtraRows = attrs.hasOwnProperty('noExtraRows'), today = $filter('date')(new Date(), 'yyyy-MM-dd');
        ;
        // 一周伊始于
        ...
        scope.showModal = function (dateObj) {
          ...
          scope.myModal = $modal({
            scope: scope,
            templateUrl: 'views/template/lesson-modal.html',
            title: title,
            show: false
          });
        ...
        function buildMonth(item) {
          ...
          var allDates = dateUtils.buildDates(initDate, {
              weekStartsOn: weekStartsOn,
              noExtraRows: true
            });
          var dates = [];
          for (var i = 0; i < allDates.length; i++) {
            var className = 'pickadate-enabled', dateObj = allDates[i], date = $filter('date')(dateObj, 'yyyy-MM-dd');
            // 非当前月的灰掉
