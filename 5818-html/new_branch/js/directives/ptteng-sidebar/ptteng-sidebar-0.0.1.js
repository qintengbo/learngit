...
angular.module('admin').directive('sidebar', function () {
  return {
    ...
    controller: [
      ...
      'recordCookies',
      'commonUtil',
      function ($scope, $rootScope, $state, $location, roleService, managerService, moduleService, recordCookies, commonUtil) {
          ...
          if (res.data.code == 0) {
            ...
            if (sessionStorage.mineSide == '' || sessionStorage.mineSide == undefined) {
              moduleService.batchGetModule(moduleIDs).then(function (res) {
                if (res.data.code == 0) {
                  $scope.sideList = commonUtil.buildTree(res.data.data.moduleList);  // 刷新时的高亮标识
                                                                                     //markLightFromUrl();
                }
              });
            } else {
