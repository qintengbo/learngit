angular.module("admin").controller('cashAuditDetailsControl', ['$rootScope', '$scope', '$state', 'cashService', '$modal', 'commonUtil',
    function ($rootScope, $scope, $state, cashService, $modal, commonUtil) {
        ...
        //批次审核  提现审核详情中  checkStatus 1--待审核、2--通过、3--未通过
        vm.cashAudits = function () {
            vm.detailList=[];
             vm.ids=[];
            angular.forEach(vm.list,function(data){
                    if(data.choice && data.checkStatus==1){
                        vm.ids.push(data.id)
                    }
            });
            if(vm.ids.length==0){
        ...
        //审核确认model
        vm.checkMoneyModalFun= function (id) {
            vm.detailList=[]

            //获取到操作的id
            vm.cashAudit.data.withdrawalsId = id;
            vm.checkMoneyModal.show();
            delete vm.cashAudit.data.reason;
        ...
        vm.agree= function () {
            //type1为审核状态 2 为提现状态
            vm.cashAudit.data.type=1
            delete vm.cashAudit.data.reason;
            //约定1为审核通过
            vm.cashAudit.data.status = 1;

            cashService.cashAudit(vm.cashAudit.data).then(function (res) {
                if (res.data.code === 0) {
                    vm.cashAuditModal.hide();
                    vm.checkMoneyModal.hide()
                    vm.cashDetailsGo()
