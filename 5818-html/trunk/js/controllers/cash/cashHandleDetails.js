        ...
        function ($rootScope, $scope,$modal, $state, cashService, commonUtil) {
            ...
            vm.checkCashHandleList={
                ...
                getCashHandleListDetail:function(){
                    ...
                    vm.ids=[];
                    angular.forEach(vm.list,function(data){
                        if(data.choice && data.dealStatus==4){
                            vm.ids.push(data.id)
                        }
                    });
                    if(vm.ids.length==0){
            ...
            //单个 查看详情
                vm. getCashHandleOneDetail=function(id){
                vm.checkCashHandle.data.withdrawalsId = id;
                vm.checkCashHandle.data.type=2;
                vm.detailList=[];
                vm.checkMoneyModal.show();
                delete vm.checkCashHandle.data.reason;
                //获取某个id的详情
                cashService.getCashAuditOneDetails(id).then(function(res){
                    if(res.data.code==0){
                        vm.detailList.push(res.data.data);
            ...
            //单个 通过的方法
            vm.pass=function(){
                vm.checkCashHandle.data.status=1;
                delete vm.checkCashHandle.data.reason;

                cashService.cashAudit(vm.checkCashHandle.data).then(function (res) {
                    if (res.data.code === 0) {
                        vm.cashHandleModal.hide();
                        vm.checkMoneyModal.hide()
                        vm.cashDetailsGo()
            ...
            //单个 拒绝的方法
                ...
                vm.fail=function(){
                vm.checkCashHandle.data.status=2;
