...
////只能输入字母
angular.module('admin').directive('onlyEnglish', function () {
  return {
    require: 'ngModel',
    restrict: 'EA',
    scope: {
    ...
    link: function (scope, ele, attrs, modelCtrl) {
      modelCtrl.$parsers.push(function (inputValue) {
        ...
        var maxLength = +scope.maxLength;
        var min = +scope.min;
        var transformedInput = inputValue.replace(/^[\u4e00-\u9fa50-9]*$/g, '');
        if (maxLength && inputValue.length > maxLength) {
          transformedInput = inputValue.slice(0, maxLength);
        }
        if (max && +transformedInput > max) {
...
//只能输入数字和 "/"符号
angular.module('admin').directive('onlyLotteryNum', function () {
  return {
    require: 'ngModel',
    restrict: 'EA',
    scope: {
