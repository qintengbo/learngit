angular.module("admin").controller('winCount', ['$rootScope','commonUtil','$scope','$state','wincountService','parameterService',
    function($rootScope,commonUtil,$scope,$state,wincountService,parameterService) {
        var vm = this;
        vm.params = $state.params;
        if(vm.params&&vm.params.startAt&&vm.params.startAt !=null&&vm.params.startAt !=undefined){
            vm.params.startAt = parseInt($state.params.startAt);
        }
        if(vm.params&&vm.params.endAt&&vm.params.endAt !=null && vm.params.endAt !=undefined){
        ...
        //获取中奖统计列表
                ...
                if(vm.params&&(vm.params.endAt ||vm.params.startAt ||vm.params.typeCode)){
        ...
        vm.search =function () {
            ...
            if(vm.params.endAt){
                var timeString=String(vm.params.endAt);
                var str = timeString.substring(timeString.length - 1, timeString.length);
                if(str!=9){
                    vm.params.endAt=vm.params.endAt+86400000 - 1;
                }
            }
